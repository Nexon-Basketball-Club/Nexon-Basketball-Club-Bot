 ...
 *
 * **회원번호는 이름이 겹칠 때만 붙인다.** 평소엔 단톡방에 코드가 줄줄이 뜨는 게 지저분하고,
 * 겹칠 때는 이름만으로 두 사람을 가를 방법이 없다. 서로 다른 두 미납자가 같은 이름으로
 * 나갔던 적이 있는데, 그때 아무도 이상하다고 못 느낀 게 이 단서가 없어서였다.
 */
...
function helpText() {
  // **`!참석`/`!취소`는 일부러 빼놨다.** 명령어는 살아 있고 동작한다 — 아직 회원들에게
  // 열 준비가 안 돼서 목록에만 안 띄운다. 열 때 두 줄을 되살리고 sim/snapshots/basic.txt를
  // 갱신하면 된다.
  return (
    "사용법\n" +
    " !회비 <이름>      회원·회비·게스트비\n" +
